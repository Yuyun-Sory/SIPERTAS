...
.siswa-banner{height:80px;background:linear-gradient(135deg,#2563eb,#7c3aed);position:relative;}
...
.btn-submit{flex:1;display:flex;align-items:center;justify-content:center;gap:8px;
    padding:11px;border:none;border-radius:var(--r-sm);background:var(--accent);
    font-family:var(--sans);font-size:14px;font-weight:700;color:#fff;cursor:pointer;
    transition:background .15s;box-shadow:0 2px 8px rgba(37,99,235,.3);}
...
.progress-fill{height:100%;background:linear-gradient(90deg,var(--accent),#6366f1);border-radius:3px;transition:width .3s;}
...
<div class="ph">
    <div>
        ...
        <p class="ph-sub">Mengisi nilai untuk: <strong>{{ $siswa->nama }}</strong></p>
    </div>
</div>
<hr class="divider">

<form method="POST" action="{{ route('nilai.store', $siswa) }}" id="nilaiForm">
@csrf
...
<div class="form-layout">
            ...
            <div class="siswa-body">
                ...
                <div class="siswa-meta">
                    ...
                    <div class="meta-row">
                        ...
                        <span class="meta-val">{{ $kriterias->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div>
        <div class="form-card">
            <div class="form-head">
                <div class="form-head-title">📋 Form Penilaian</div>
                <div class="form-head-sub">Pilih level (1–4) untuk setiap kriteria. Semua kriteria wajib diisi.</div>
            </div>

            {{-- PROGRESS --}}
            <div class="progress-wrap">
                <div class="progress-info">
                    <span id="progressLabel">0 / {{ $kriterias->count() }} kriteria diisi</span>
            ...
            <div class="form-body">
                ...
                <div class="kriteria-block" id="block-{{ $kriteria->id }}" data-kriteria="{{ $kriteria->id }}">
                    <div class="kriteria-header">
                        ...
                        <span class="valid-indicator" id="vi-{{ $kriteria->id }}">⬜</span>
                    </div>
                    <div class="level-options">
                        {{-- Level 4 --}}
                        @php $sub4 = $kriteria->subKriterias->firstWhere('level', 4); @endphp
                        <div class="level-opt lv-opt-4">
                            <input type="radio" name="nilai[{{ $kriteria->id }}]"
                                   id="k{{ $kriteria->id }}_l4" value="4"
                                   onchange="updateProgress()">
                            <label class="level-label" for="k{{ $kriteria->id }}_l4">
                                <span class="level-num" style="color:var(--lv4-tx)">4</span>
                                <span class="level-text">Sangat Baik</span>
                                @if($sub4)
                                <span class="level-desc">{{ $sub4->nama }}</span>
                                @endif
                            </label>
                        </div>
                        {{-- Level 3 --}}
                        @php $sub3 = $kriteria->subKriterias->firstWhere('level', 3); @endphp
                        <div class="level-opt lv-opt-3">
                            <input type="radio" name="nilai[{{ $kriteria->id }}]"
                                   id="k{{ $kriteria->id }}_l3" value="3"
                                   onchange="updateProgress()">
                            <label class="level-label" for="k{{ $kriteria->id }}_l3">
                                <span class="level-num" style="color:var(--lv3-tx)">3</span>
                                <span class="level-text">Baik</span>
                                @if($sub3)
                                <span class="level-desc">{{ $sub3->nama }}</span>
                                @endif
                            </label>
                        </div>
                        {{-- Level 2 --}}
                        @php $sub2 = $kriteria->subKriterias->firstWhere('level', 2); @endphp
                        <div class="level-opt lv-opt-2">
                            <input type="radio" name="nilai[{{ $kriteria->id }}]"
                                   id="k{{ $kriteria->id }}_l2" value="2"
                                   onchange="updateProgress()">
                            <label class="level-label" for="k{{ $kriteria->id }}_l2">
                                <span class="level-num" style="color:var(--lv2-tx)">2</span>
                                <span class="level-text">Cukup</span>
                                @if($sub2)
                                <span class="level-desc">{{ $sub2->nama }}</span>
                                @endif
                            </label>
                        </div>
                        {{-- Level 1 --}}
                        @php $sub1 = $kriteria->subKriterias->firstWhere('level', 1); @endphp
                        <div class="level-opt lv-opt-1">
                            <input type="radio" name="nilai[{{ $kriteria->id }}]"
                                   id="k{{ $kriteria->id }}_l1" value="1"
                                   onchange="updateProgress()">
                            <label class="level-label" for="k{{ $kriteria->id }}_l1">
                                <span class="level-num" style="color:var(--lv1-tx)">1</span>
                                <span class="level-text">Kurang</span>
                                @if($sub1)
                                <span class="level-desc">{{ $sub1->nama }}</span>
                                @endif
                            </label>
                        </div>
                    </div>
...
function updateProgress() {
    const radios = document.querySelectorAll('.kriteria-block');
    let filled = 0;

    radios.forEach(block => {
